...
> **定位：能力库 / 学习库**——与 ML / DL / LLM 能力库平行，回答 QS 面试与研究方法论；
> **不属于 QS 项目集**（alpha signal 研究、portfolio 构建是项目集，本仓库不是其中的项目）。
...
| 线 | 是什么 | 产出 |
| --- | --- | --- |
| QS 项目集 | alpha 信号研究、组合构建等真实研究项目 | 策略、研报、track record |
| **能力库（本仓库 + ML/DL/LLM）** | 面试与研究方法论 | **知识、题卡、方法** |

本仓库是能力库：**不产出策略项目**，而是让项目集可信——尤其 04 回测严谨性，回答"凭什么相信这个信号不是运气"。

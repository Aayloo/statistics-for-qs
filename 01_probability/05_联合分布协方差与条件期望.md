...
> 注：$\rho$ 只度量**线性**关系；独立 → 不相关，但反过来不成立：**不相关并不蕴含独立**（例：$X \sim U(-1,1)$，$Y=X^2$）。
...
$N \sim \text{Poisson}(\lambda)$，$X_i$ iid 且独立于 $N$：
...
1. 独立一定不相关，反过来？ → 否（$Y = X^2$ 例）。
2. $\mathrm{Cov}(X,Y)=0$ 与 $\mathrm{Var}(X+Y)=\mathrm{Var}(X)+\mathrm{Var}(Y)$ 的关系？ → 等价。

**核心**

3. $\rho=0.5$ 两资产等权组合波动（各 20%）？ → ≈17.3%。
